...
def submit_pizza_order(club_id):
    ...
    if not amount or not address:
        return jsonify({'error': 'Amount and address are required'}), 400
    ...
    try:
        ...
        if amount > float(club.balance):
            return jsonify({'error': 'Amount exceeds club balance'}), 400
    ...
    try:
        from decimal import Decimal
        club.balance -= Decimal(str(amount))
        db.session.commit()
        
        # Log the pizza order (you could create a PizzaOrder model here for tracking)
        app.logger.info(f"Pizza order submitted for club {club.name} (ID: {club.id}) by user {current_user.username} (ID: {current_user.id}). Amount: ${amount}, Address: {address}")
        ...
        db.session.rollback()
        app.logger.error(f"Error processing pizza order: {str(e)}")
