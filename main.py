...
try:
    ...
    print("Warning: psycopg2 not available, attempting to install...")
    import subprocess
    try:
        # Try installing the latest version first
        subprocess.check_call(['pip', 'install', '--upgrade', '--no-cache-dir', 'psycopg2-binary'])
        import psycopg2
...
try:
    ...
    db = SQLAlchemy(app)
    db_available = True
    print("Database setup successful with PostgreSQL support")
except ImportError as e:
    print(f"Database setup failed - missing dependencies: {e}")
    db_available = False
except Exception as e:
    print(f"Database initialization failed: {e}")
    db_available = False

# Initialize login manager if database is available
if db_available:
    try:
...
if db_available and db:
    ...
    # Create placeholder classes when database is not available
    class User:
        pass
    class Club:
        pass
